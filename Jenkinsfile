...
properties([
    ...
    parameters([
        ...
        string(name: 'SWV_BACKEND_URL', defaultValue: 'http://swv-backend-url.example.com/api/notification', description: 'SWV Backend Notification URL')
        string(name: 'EMAIL_TO', defaultValue: 'jaehokim1005@g.hongik.ac.kr', description: 'Email to send notification')
    ])
])
...
pipeline {
    ...
    tools {
        'org.sonarsource.scanner.cli.SonarScanner' "${env.SONAR_SCANNER_TOOL}"
    }
    ...
    post {
        ...
        success {
            ...
            // 성공 알림 이메일 전송
            mail to: string(params.NOTIFY_EMAIL),
